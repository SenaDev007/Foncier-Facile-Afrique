...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const pathname = `/annonces/${params.slug}`
  if (!annonce) {
  ...
  const descSource =
    annonce.description?.trim() ?? `${annonce.titre} — ${annonce.localisation}. Annonce immobilière vérifiée Foncier Facile Afrique.`
  return publicPageMetadata({
    title: annonce.titre,
    description: truncateMetaDescription(descSource),
    pathname,
    ogImage: annonce.photos[0]?.url ?? null,
    noindex: annonce.statut !== 'EN_LIGNE',
    keywords: [annonce.localisation, annonce.titre.slice(0, 40)],
  })
}
  ...
  return (
    <div className="bg-ffa-ink min-h-screen py-10">
      <div className="container-site">
        <nav className="mb-6 text-sm text-ffa-fg-muted" aria-label="Fil d'Ariane">
          <Link href="/" className="hover:text-ffa-gold">Accueil</Link>
          {' / '}
          <Link href="/annonces" className="hover:text-ffa-gold">Annonces</Link>
          {' / '}
          <span className="text-ffa-fg">{annonce.titre}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-ffa-elevated border border-ffa-divider rounded-2xl p-6">
              <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                <div>
                  ...
                  <h1 className="font-heading text-2xl font-bold text-ffa-fg">{annonce.titre}</h1>
                </div>
                <p className="font-heading text-2xl font-bold text-ffa-gold whitespace-nowrap">{formatPrice(annonce.prix)}</p>
              </div>

              <div className="flex flex-wrap gap-4 text-ffa-fg-muted text-sm mb-6">
                {annonce.localisation && (
                  <span className="flex items-center gap-1.5"><MapPin className="h-4 w-4 text-ffa-gold" aria-hidden="true" />{annonce.localisation}</span>
                )}
                {annonce.surface && (
              ...
              <div className="pt-4 border-t border-ffa-divider">
                <ShareButtons
              ...
              {annonce.description && (
                <div>
                  <h2 className="font-heading font-semibold text-ffa-fg text-base mb-3">Description</h2>
                  <p className="text-ffa-fg-muted leading-relaxed whitespace-pre-line">{annonce.description}</p>
                </div>
              )}
            </div>

            <div className="rounded-xl overflow-hidden border border-ffa-divider">
              <h2 className="font-heading font-semibold text-ffa-fg text-sm mb-3 px-1">Localisation</h2>
          ...
          <div className="space-y-4">
            <div className="bg-ffa-elevated border border-ffa-divider rounded-2xl p-6 sticky top-20">
              <h2 className="font-heading font-semibold text-ffa-fg text-base mb-4">Demande d’information</h2>
              <LeadFormAnnonce
                annonceId={annonce.id}
                annonceReference={annonce.reference}
                annonceTitre={annonce.titre}
              />
              <div className="mt-4 pt-4 border-t border-ffa-divider space-y-2">
                <a
                  href={getWhatsAppUrl('22996901204', whatsappMsg)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full bg-[#D4A843] text-[#1C1C1E] font-semibold py-3 rounded-xl hover:bg-[#B8912E] transition-colors"
                >
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-2 w-full border border-ffa-gold text-ffa-gold font-semibold py-3 rounded-xl hover:bg-ffa-gold/12 transition-colors"
                >
                  <Phone className="h-4 w-4" aria-hidden="true" />
                  +229 96 90 12 04
                </a>
              </div>
              <p className="text-xs text-ffa-fg-muted text-center mt-3">Réponse sous 24h garantie</p>
            </div>
          </div>
        </div>
      </div>
