...
import { Shield, FileCheck, Search, Users, BadgeCheck, Home, Sparkles, Building2 } from 'lucide-react'
...
export function FeatureCarousel({ features }: { features: FeatureItem[] }) {
  const [step, setStep] = useState(0)
  const [isPaused, setIsPaused] = useState(false)

  const data = features.map((f) => ({ ...f, icon: f.icon ?? ICON_MAP.Shield }))
  const currentIndex = ((step % data.length) + data.length) % data.length
        ...
        <div className="w-full lg:w-[40%] min-h-[350px] md:min-h-[450px] lg:h-full relative z-30 flex flex-col items-start justify-center overflow-hidden px-8 md:px-16 lg:pl-16 bg-[#62B2FE]">
          ...
          <div className="relative w-full h-full flex items-center justify-center lg:justify-start z-20">
            {data.map((feature, index) => {
              ...
              const wrappedDistance = wrap(-(data.length / 2), data.length / 2, distance)
